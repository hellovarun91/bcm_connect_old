...
const ObjectOptions = memo(({ object, onOptionSelect, isVisible = true, onAnimationComplete }) => {
  ...
  console.log(`ObjectOptions rendered for object: ${object.name || object.id} (isVisible: ${isVisible})`);
  
  // Get current location to check if we're on the canvas page
    ...
    return object.options.map((option, index) => {
      ...
      return (
        <div 
          ...
          style={style}
          onClick={(e) => {
            console.log(`ObjectOptions: Option clicked: ${option.label} for object: ${object.name || object.id}`);
  ...
  // If there's no object or options, or if we're not on the canvas page, don't render anything
  if (!object || !object.options || object.options.length === 0) return null;
  
  // Only show options on the canvas page
